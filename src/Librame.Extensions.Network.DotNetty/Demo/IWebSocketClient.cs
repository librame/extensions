...
namespace Librame.Extensions.Network.DotNetty.Demo
{
    ...
    /// WebSocket 客户端接口。
    /// </summary>
    public interface IWebSocketClient : IChannelService
    {
        /// <summary>
        /// 异步启动。
        /// </summary>
        ...
        /// <param name="channelHandlerFactory">给定的 <see cref="IChannelHandler"/> 工厂方法。</param>
        /// <param name="configureProcess">给定的配置处理方法。</param>
        /// <param name="host">给定要启动的主机（可选；默认使用选项配置）。</param>
        /// <param name="port">给定要启动的端口（可选；默认使用选项配置）。</param>
        /// <returns>返回一个异步操作。</returns>
        Task StartAsync<TChannelHandler>(Func<Uri, TChannelHandler> channelHandlerFactory,
