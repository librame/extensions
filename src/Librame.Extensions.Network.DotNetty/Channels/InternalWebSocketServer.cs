...
namespace Librame.Extensions.Network.DotNetty
{
    ...
    internal class InternalWebSocketServer : AbstractChannelService<InternalWebSocketServer>, IWebSocketServer
    {
        ...
        public InternalWebSocketServer(ISigningCredentialsProvider provider,
        ...
        {
            handlerFactory = handlerFactory.AsValueOrDefault(() => new InternalWebSocketServerHandler(this));
            ...
            try
            {
                ...
                Logger.LogInformation("Open your web browser and navigate to "
                    + $"{(_serverOptions.UseSSL ? "https" : "http")}"
                    + $"://{bootstrapChannel.LocalAddress.ToString()}:{port}/");
                Logger.LogInformation("Listening on "
                    + $"{(_serverOptions.UseSSL ? "wss" : "ws")}"
                    + $"://{bootstrapChannel.LocalAddress.ToString()}:{port}/websocket");
