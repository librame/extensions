...
{
    public class MaterialForm : System.Windows.Forms.Form, IMaterialControl
    {
        /// <summary>
        /// 获取或设置深度。
        /// </summary>
        ...
        private const int STATUS_BAR_HEIGHT = 34;
        ...
        private Rectangle textButtonBounds;
        ...
        //public bool EnableActionBar { get; set; }

        public MaterialForm()
            : this(STATUS_BAR_HEIGHT)
        {
        }
        public MaterialForm(int statusBarHeight)
        {
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == WM_LBUTTONDBLCLK)
            {
                ...
                !(minButtonBounds.Contains(PointToClient(Cursor.Position)) || maxButtonBounds.Contains(PointToClient(Cursor.Position)) ||
                xButtonBounds.Contains(PointToClient(Cursor.Position))))) //|| actionBarBounds.Contains(PointToClient(Cursor.Position))
            {
                if (headerMouseDown)
                {
                    Maximized = false;
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            minButtonBounds = new Rectangle((Width - Skin.Settings.FormPadding / 2) - 3 * STATUS_BAR_BUTTON_WIDTH, 0, StatusBarHeight, StatusBarHeight);
            maxButtonBounds = new Rectangle((Width - Skin.Settings.FormPadding / 2) - 2 * STATUS_BAR_BUTTON_WIDTH, 0, StatusBarHeight, StatusBarHeight);
            xButtonBounds = new Rectangle((Width - Skin.Settings.FormPadding / 2) - STATUS_BAR_BUTTON_WIDTH, 0, StatusBarHeight, StatusBarHeight);

            // 初始化顶部状态栏矩形
            statusBarBounds = new Rectangle(0, 0, Width, StatusBarHeight);

            var x = 4;
            var iconSize = new Size(24, 24);
            if (!ReferenceEquals(Icon, null))
            {
                int y = (StatusBarHeight - iconSize.Height) / 2;
                iconButtonBounds = new Rectangle(x + y, y, iconSize.Width, iconSize.Height);
                // 标题文本
                textButtonBounds = new Rectangle(iconButtonBounds.Width + 2 * x + 2 * y, y - 2, minButtonBounds.X - StatusBarHeight - 2 * y, Icon.Height);
            }
            else
            {
                textButtonBounds = new Rectangle(2 * 4, 4, minButtonBounds.X - 8, 32);
            }

            // 初始化上部动作栏矩形
            //actionBarBounds = new Rectangle(0, StatusBarHeight, Width, ActionBarHeight);
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // 绘制顶部状态栏
            g.FillRectangle(Skin.Scheme.Brushes.Primary, statusBarBounds);

            // 绘制上部动作栏
            //if (EnableActionBar)
            //{
            //    g.FillRectangle(Skin.Scheme.Brushes.Primary, actionBarBounds);

            //    //Draw border
            //    using (var borderPen = new Pen(Skin.Scheme.Texts.Dividers.Color, 1))
            //    {
            ...
            bool showMin = MinimizeBox && ControlBox;
            bool showMax = MaximizeBox && ControlBox;
            var hoverBrush = Skin.Scheme.FlatButton.BackgroundHover.Brush;
            var downBrush = Skin.Scheme.FlatButton.BackgroundPressed.Brush;

            // 此行代码表示有矩形背景
            //if (ShowIcon && !ReferenceEquals(Icon, null))
            //    g.FillRectangle(hoverBrush, iconButtonBounds);
            
            //if (!String.IsNullOrEmpty(Text))
            //    g.FillRectangle(hoverBrush, textButtonBounds);
            ...
            // 图标
            if (ShowIcon && !ReferenceEquals(Icon, null))
                g.DrawIcon(Icon, iconButtonBounds);

            if (!String.IsNullOrEmpty(Text))
            {
                g.DrawString(Text, Skin.Scheme.Fonts.Medium11, Skin.Scheme.Brushes.TextShade, textButtonBounds,
                    new StringFormat { Trimming = StringTrimming.EllipsisCharacter, LineAlignment = StringAlignment.Center });
            }
            
            // 绘出顶部状态栏的控制按钮
            using (var formButtonsPen = new Pen(Skin.Scheme.ActionBar.Secondary.Color, 2))
            {
                // Minimize button.
                if (showMin)
                {
            ...
            //if (EnableActionBar)
            //{
            //    g.DrawString(Text, SkinManager.MEDIUM_12, Skin.Scheme.Brushes.TextShade,
            //        new Rectangle(Skin.FormPadding, StatusBarHeight, Width, ActionBarHeight),
