...
namespace Librame.Extensions.Encryption.Identifiers
{
    ...
    {
        private static readonly IAlgorithmConverter _converter
        ...
        private static bool TryParseIdentifier(string identifier, out byte[] buffer)
        {
            try
            {
                ...
                buffer = _converter.ConvertFrom(signedToken.DataSegments[0]);
                return true;
            }
            catch (Exception)
            {
        ...
        /// 隐式转换为字符串形式。
        /// </summary>
        /// <param name="descriptor">给定的 <see cref="SecurityIdentifier"/>。</param>
        public static implicit operator string(SecurityIdentifier descriptor)
            => descriptor?.ToString();
